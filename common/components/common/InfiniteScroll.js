...
const InfiniteScroll = (C, options = { limit: 10, collection: null }) => {
    return (props) => {
        ...
        const screenProps = props.navigation.getScreenProps();
        ...
        const retrieveData = async () => {
            ...
            try {
                ...
                const promiseData = Promise.all(database.collection(options.collection).where('apartmentID', '==', screenProps.apartmentID).orderBy('modifiedDate', 'desc').get());
                let dd = [];
                ...
                //     .where('apartmentID', '==', screenProps.apartmentID)
        ...
        const setUp = () => {
            ...
            if (options.collection === 'dailyTasks') {
                ...
                cachedData = model.getApartmentTickets(screenProps.apartmentID);
            }
